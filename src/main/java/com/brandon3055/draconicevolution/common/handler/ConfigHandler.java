    ...
    public static void syncConfig() {
        ...
        try {
            ...
            bowBlockDamage = config.get(
                    ...
                    "Bow Block Damage",
                    true,
                    "Dose Draconic bow explosion damage blocks").getBoolean(true);
            ...
            passiveSoulDropChance = config.get(
                    ...
                    1,
                    Integer.MAX_VALUE).getInt(800);
            cometRarity = config.get(
                    Configuration.CATEGORY_GENERAL,
                    "Ender Comet Rarity",
                    10000,
                    "Ender Comet has a 1 in {this number} chance to spawn in each chunk").getInt(10000);
            generateEnderComets = config.get(
                    Configuration.CATEGORY_GENERAL,
                    "Generate Ender Comets",
                    true,
                    "Should Ender comets be generated").getBoolean(true);
            generateChaosIslands = config.get(
                    Configuration.CATEGORY_GENERAL,
                    "Generate Chaos Islands",
                    true,
                    "Should Chaos Islands be generated").getBoolean(true);
            pigmenBloodRage = config
                    .get(Configuration.CATEGORY_GENERAL, "Pigmen Blood Rage", true, "Is Pigmen blood rage active")
                    .getBoolean(true);
            disableLore = config.get(
                    Configuration.CATEGORY_GENERAL,
                    "Disable Item Lore",
                    false,
                    "Set to true to disable all item lore").getBoolean(false);
            invertDPDSB = config.get(
                    Configuration.CATEGORY_GENERAL,
                    "InvertDPDSB",
                    false,
                    "Invert Dislocator Pedestal display name shift behavior").getBoolean(false);
            ...
            disableOreSpawnEnd = config.get(
                    ...
                    false,
                    "Set to true to prevent draconium ore from spawning in the end").getBoolean(false);
            disableOreSpawnNether = config.get(
                    Configuration.CATEGORY_GENERAL,
                    "Disable Ore Spawn (Nether)",
                    false,
                    "Set to true to prevent draconium ore from spawning in the nether").getBoolean(false);
            ...
            maxPlayerSpeed = config.get(
                    ...
                    "Player speed cap",
                    10D,
                    "Limits the max speed of players. Recommend between 0.5 - 1.0 for servers").getDouble(10D);
            ...
            speedLimitops = config.get(
                    ...
                    false,
                    "Dose the speed limit effect ops").getBoolean(false);
            rapidlyDespawnMinedItems = config
                    .get(
                            Configuration.CATEGORY_GENERAL,
            ...
            sumonRitualAccelerated = config.get(
                    ...
                    "Fast Summon Ritual",
                    false,
                    "If true the resurrection ritual will be a lot faster").getBoolean(false);
            useOldD2DToolTextures = config.get(
                    Configuration.CATEGORY_GENERAL,
                    "Use old 2D tool textures",
                    false,
                    "If true the 3D tool models will be replaced with standard 2D textures").getBoolean(false);
            ...
            enableFlight = config.get(
                    ...
                    "Enable Flight",
                    true,
                    "Set this to false to disable flight given by draconic armor.").getBoolean(true);
...
/*
 * { //config.load(); disableSunDial = config.getInt("Disable Sun Dial", Configuration.CATEGORY_GENERAL, disableSunDial,
 * 0, 2, "Disable Sun Dial 0:Default, 1:Disable recipe, 2:Disable completely"); disableXrayBlock =
 * config.getInt("Disable Xray Block", Configuration.CATEGORY_GENERAL, disableXrayBlock, 0, 2,
 * "Disable Distortion Flame 0:Default, 1:Disable recipe, 2:Disable completely"); teleporterUsesPerPearl =
 * config.getInt("Teleporter Uses PerPearl", Configuration.CATEGORY_GENERAL, teleporterUsesPerPearl, 0,
 * Integer.MAX_VALUE, "Charm of Dislocation uses per Ender pearl"); bowBlockDamage =
 * config.getBoolean(Configuration.CATEGORY_GENERAL, "Bow Block Damage", bowBlockDamage,
 * "Dose Draconic bow explosion damage blocks"); spawnerListType = config.getBoolean("spawner", "listType",
 * spawnerListType,
 * "Sets weather the spawner list is a white list or a black list (true = white list false = black list)"); spawnerList
 * = config.getStringList("Spawn List", "spawner", defaultSpawnerList,
 * "List of names that will be ether accepted or rejected by the spawner depending on the list type"); soulDropChance =
 * config.getInt("soulDropChance", "spawner", soulDropChance, 1, Integer.MAX_VALUE,
 * "Mobs have a 1 in this number chance to drop a soul"); updateFix = config.getBoolean(Configuration.CATEGORY_GENERAL,
 * "Update Fix", updateFix,
 ...
 * //dislocator_Max_Range = config.get("Long Range Dislocator", "Admin Dislocator Max Range",
 * 29000000).getInt(29000000); admin_dislocator_Min_Range = config.getInt("Dislocator Min Range",
 * "long range dislocator", admin_dislocator_Min_Range, 0, 100000, ""); admin_dislocator_Max_Range =
 * config.getInt("Dislocator Max Range", "long range dislocator", admin_dislocator_Max_Range, 0, 100000, "");
 * admin_dislocator_Detect_Range = config.getInt("long range dislocator", "Admin Dislocator Detect Range",
 * admin_dislocator_Detect_Range, 0, 100, ""); disable_LRD = config.getInt("Disable Dislocator",
 * "long range dislocator", disable_LRD, 0, 2,
